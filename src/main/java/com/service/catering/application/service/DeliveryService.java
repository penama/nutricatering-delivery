...
public class DeliveryService extends BaseCommandHandler {
  ...
  @Autowired
  private ProducerEventDeliveryUpdateStatusFoodPackageService
      producerEventDeliveryUpdateStatusFoodPackageService;
  ...
  public DeliveryDto newDelivery(DeliveryDto deliveryDto) throws Exception {
    ...
    producerEventDeliveryUpdateStatusFoodPackageService.producerDeliveryUpdate(
        deliveryEntity, historyDeliveryEntity);
    return DeliveryUtil.deliveryEntityToDeliveryDto(deliveryEntity);
  }

  public void updateDeliveryStatus(DeliveryUpdateStatusDto deliveryUpdateStatusDto)
      throws Exception {
    DeliveryEntity deliveryEntity =
        deliveryServiceRepository.queryDeliveryId(deliveryUpdateStatusDto.getDeliveryId());
  ...
  public List<DeliveryDto> getDeliveryByPersonId(String personId) throws Exception {
    List<DeliveryEntity> deliveryEntities =
        deliveryServiceRepository.queryDeliverysByPersonId(personId);
    List<DeliveryDto> deliveryDtos = new ArrayList<>();
    for (DeliveryEntity deliveryEntity : deliveryEntities) {
      deliveryDtos.add(DeliveryUtil.deliveryEntityToDeliveryDto(deliveryEntity));
    }
  ...
  public DeliveryStatusDto getDeliveryByFoodPackageId(String foodPackageId) throws Exception {
    ...
    List<HistoryDeliveryEntity> historyDeliveryEntities =
        historyDeliveryServiceRepository.getHistoryDeliveryByDeliveryId(deliveryEntity.getId());
    ...
    for (HistoryDeliveryEntity historyDeliveryEntity : historyDeliveryEntities) {
      historyDeliveries.add(
          HistoryDeliveryUtil.historyDeliveryEntityToHistoryDeliveryDto(historyDeliveryEntity));
    }
